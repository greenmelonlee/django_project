...
def tag_posts(request, slug): # URL로 전달받은 slug 변수를 아규먼트로 사용
    ...
    if slug == 'no tag':
        ...
        # posts = Post.objects.filter(tag=tag).select_related()
        # 쿼리 최적화를 위해서 ForeignKey나 일대일 관계로 연결된 테이블 데이털를 미리 가져오는 메서드
...
class CommentUpdate(LoginRequiredMixin, UpdateView): #updateview를 상속받아서 씀
    ...
    def dispatch(self, request, *args, **kwargs): # 방문자가 GET으로 요청했는데 POST로 요청했는지 확인하는 기능
        if request.user.is_authenticated and request.user == self.get_object().author:
            ...
            raise PermissionDenied # 권한 없는 사람이 글 수정하려 하면 403에러가 남
...
def delete_comment(request, pk):
    ...
    if request.user.is_authenticated and request.user == comment.author:
        comment.delete()
        return redirect(post.get_absolute_url()) # /blog/7
    else:
        raise PermissionDenied
